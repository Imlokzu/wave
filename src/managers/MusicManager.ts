...
export class MusicManager {
  ...
  async uploadTrack(
    ...
    try {
      ...
      const fileName = `${trackId}_${originalName}`;
      const filePath = `music/${userId}/${fileName}`;

      // Get file data (buffer or File)
      const fileData = file.buffer || file;

      // Upload to Supabase Storage (flux-music bucket)
   ...
   */
  async extractMetadata(file: any): Promise<{
    title: string;
    artist: string;
    album: string;
    duration: number;
    ...
    try {
      // For buffer-based files (from multer)
      if (file.buffer) {
      ...
      const filename = file.originalname || file.name || 'Unknown Track';
      return {
  ...
  async deleteFromCloud(trackId: string, userId: string): Promise<void> {
    try {
      ...
      // Delete from storage (flux-music bucket)
