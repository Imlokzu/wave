  ...
  private async executeTool(toolCall: AIToolCall): Promise<string> {
    ...
    if (name === 'ddg_search') {
      try {
        ...
        console.log(`[DeepSeek] Searching with full content for: "${query}"${this.userRegion ? ` (region: ${this.userRegion})` : ''}`);
        
        // Use searchWithContent to get full page content (prevents hallucination)
        const results = await this.searchService.searchWithContent(query, 3, this.userRegion);
        const formattedResults = this.searchService.formatResultsWithContentForAI(results);
        
        console.log(`[DeepSeek] Search completed with full content from ${results.length} pages`);
