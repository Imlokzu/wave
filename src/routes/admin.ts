...
export function createAdminRouter(userManager: IUserManager): Router {
  ...
  /**
   * GET /api/admin/reports/messages
   * Get all message reports
   */
  router.get('/reports/messages', async (req: AdminAuthenticatedRequest, res: Response) => {
    try {
      const status = req.query.status as string;
      const { createClient } = await import('@supabase/supabase-js');
      const supabase = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_KEY!);
      ...
      let query = supabase
        ...
        .select(`
          *,
          reporter:flux_users!message_reports_reporter_id_fkey(id, username, nickname),
          reported_user:flux_users!message_reports_reported_user_id_fkey(id, username, nickname, is_banned)
        `)
        .order('created_at', { ascending: false });
  ...
  /**
   * PATCH /api/admin/reports/messages/:reportId
   * Update message report status
   */
  router.patch('/reports/messages/:reportId', async (req: AdminAuthenticatedRequest, res: Response) => {
    try {
      const { reportId } = req.params;
      const { status, adminNotes } = req.body;
      ...
      res.json({ message: 'Message report updated', report: data });
    } catch (error: any) {
