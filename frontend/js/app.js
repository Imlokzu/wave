...
class App {
  ...
  setupSidebarSearch() {
    ...
    searchInput.addEventListener('input', (e) => {
      ...
      searchTimeout = setTimeout(async () => {
        try {
          const response = await fetch(`${window.API_URL}/api/users/search?q=${encodeURIComponent(query)}`);
  ...
  async checkUserOnlineStatus(username) {
    try {
      ...
      const response = await fetch(`${window.API_URL}/api/users/search?q=${encodeURIComponent(username)}`);
      if (!response.ok) {
        return false;
      }
  ...
  handleSetStatus() {
    ...
    if (choice) {
      ...
      if (index >= 0 && index < statuses.length) {
        ...
        if (userId) {
          fetch(`${window.API_URL}/api/users/${userId}/status`, {
            method: 'PUT',
  ...
  async openFilePicker(acceptTypes = 'image/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip,.rar,.7z,.gif,.mp3,.wav,.ogg,audio/*') {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const response = await fetch(`${window.API_URL}/api/rooms/${roomId}/${endpoint}`, {
          method: 'POST',
          body: formData
        });
  ...
  async handleAddFriendClick() {
    ...
    try {
      ...
      const response = await fetch(`${window.API_URL}/api/users/search?q=${encodeURIComponent(username.trim())}`);
  ...
  async handleRegenerateRoomCode() {
    ...
    try {
      ...
      const response = await fetch(`${window.API_URL}/api/rooms/${roomId}/regenerate-code`, {
        method: 'POST',
        headers: {
