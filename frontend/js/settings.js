...
async function getSubscriptionStatus() {
  ...
  const response = await fetch(`${window.API_URL}/api/settings', {
    headers: {
...
async function upgradeToPro() {
  const authToken = localStorage.getItem('authToken');
  const response = await fetch(`${window.API_URL}/api/settings/upgrade-to-pro', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${authToken}`
    }
...
async function downgradeToFree() {
  const authToken = localStorage.getItem('authToken');
  const response = await fetch(`${window.API_URL}/api/settings/downgrade-to-free', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${authToken}`
    }
...
async function toggleSetting(setting, value) {
  ...
  try {
    const authToken = localStorage.getItem('authToken');
    if (authToken) {
      await fetch(`${window.API_URL}/api/settings/update', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
...
async function loadStorageInfo() {
  try {
    ...
    const authToken = localStorage.getItem('authToken');
    const response = await fetch(`${window.API_URL}/api/settings/storage', {
...
async function saveProfileInfo() {
  ...
  try {
    const authToken = localStorage.getItem('authToken');
    if (authToken) {
      await fetch(`${window.API_URL}/api/settings/profile', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
